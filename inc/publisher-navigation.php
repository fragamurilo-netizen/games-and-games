...
function go_verge_publisher_navigation_blueprint() {
	$subjects = array(
		...
		'tecnologia' => array( 'apps-software' => 'Apps e Software', 'ia' => 'Inteligência Artificial', 'hardware' => 'Hardware', 'celulares' => 'Celulares', 'notebooks' => 'Notebooks', 'tvs-e-monitores' => 'TVs e Monitores', 'ciencia' => 'Ciência e Mundo' ),
	...
	foreach ( go_verge_editorial_pillars() as $desk => $pillar ) {
		...
		foreach ( $subjects[ $desk ] ?? array() as $slug => $label ) {
			...
			if ( isset( $term->count ) && 0 === (int) $term->count ) {
				...
				$populated = is_array( $children ) && $children ? get_terms( array( 'taxonomy' => 'category', 'include' => $children, 'hide_empty' => true, 'number' => 1 ) ) : array();
				if ( ! is_array( $populated ) || ! $populated ) { continue; }
			}
