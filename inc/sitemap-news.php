...
/**
 * Resolve the same editorial SEO title used by the document <title> whenever
 * a per-post SEO title exists. This keeps <news:title> aligned with Search.
 */
function go_verge_news_sitemap_document_title( $post_id ) {
	...
	$provider = function_exists( 'go_verge_smart_sitemap_seo_provider' )
		? go_verge_smart_sitemap_seo_provider()
		: ( defined( 'RANK_MATH_VERSION' ) ? 'rank-math' : ( defined( 'WPSEO_VERSION' ) ? 'yoast' : 'none' ) );
	$title = '';
	if ( 'rank-math' === $provider ) {
		...
		if ( '' === $title ) {
			...
			if ( is_array( $options ) ) { $title = trim( (string) ( $options['pt_post_title'] ?? '' ) ); }
		}
		if ( '' !== $title && class_exists( '\\RankMath\\Helper' ) && is_callable( array( '\\RankMath\\Helper', 'replace_vars' ) ) ) {
			$title = (string) \RankMath\Helper::replace_vars( $title, $post );
		}
		...
		if ( '' === $title ) {
			...
			if ( is_array( $options ) ) { $title = trim( (string) ( $options['title-post'] ?? '' ) ); }
		}
	...
	/* strtr replaces complete tokens without transforming %%title%% into
	 * %Headline%. Unknown variables must not escape into machine-readable XML. */
	$title = strtr( (string) $title, array(
		'%%title%%' => $post->post_title, '%title%' => $post->post_title,
		'%%sitename%%' => get_bloginfo( 'name' ), '%sitename%' => get_bloginfo( 'name' ),
		'%%sep%%' => '|', '%sep%' => '|',
	) );
	if ( preg_match( '/%{1,2}[a-z_][^%]*%{1,2}/i', $title ) ) { $title = (string) $post->post_title; }
...
function go_verge_news_sitemap_publication_name() {
	...
	return '' !== $name ? $name : 'Game Overdrive';
}
