<?php
/** Fixed manual in-page delivery with the official provider overlays. */
...
function go_verge_ads_delivery_modes() {
	return array( 'manual_overlays' => 'Manuais + âncora e vinheta' );
}
function go_verge_ads_delivery_mode() { return 'manual_overlays'; }
...
function go_verge_ads_save_delivery_mode( $requested ) {
	...
	if ( 'manual_overlays' === $requested ) { return false; }
	return new WP_Error( 'fixed_manual_architecture', 'O tema usa exclusivamente anúncios manuais com âncora e vinheta oficiais.' );
}
...
function go_verge_ads_migrate_manual_architecture() {
	...
	$marker_key = 'go_verge_ads_manual_architecture_v1';
	$marker = get_option( $marker_key, false );
	if ( is_array( $marker ) && 1 === (int) ( $marker['version'] ?? 0 ) ) { return false; }
	$backup_key = 'go_verge_ads_manual_architecture_backup_v1';
	...
	if ( 'manual_overlays' !== get_option( 'go_verge_ads_delivery_mode', null ) ) {
		update_option( 'go_verge_ads_delivery_mode', 'manual_overlays', false );
		if ( 'manual_overlays' !== get_option( 'go_verge_ads_delivery_mode', null ) ) { return false; }
	}
	$history = get_option( 'go_verge_ads_delivery_mode_history', array() );
	$history = is_array( $history ) ? $history : array();
	$recorded = false;
	foreach ( $history as $entry ) { if ( is_array( $entry ) && 'manual-architecture-v1' === ( $entry['reason'] ?? '' ) ) { $recorded = true; break; } }
	if ( ! $recorded ) {
		$history[] = array(
			...
			'to' => 'manual_overlays', 'reason' => 'manual-architecture-v1', 'trigger' => 'administrative-upgrade',
		);
...
function go_verge_ads_delivery_mode_form() {
	...
	echo '<h3>Entrega manual + âncora e vinheta</h3><p>Arquitetura única: o tema planeja os anúncios dentro das páginas. O loader oficial também atende âncora e vinheta, respeitando o consentimento existente. Não há seletor de outro motor.</p>';
	$config = go_verge_ads_config();
	...
	echo '<p><strong>No AdSense:</strong> mantenha Auto Ads geral, âncora e vinheta oficiais ligados; banners in-page, Encontrar mais posições, Otimizar anúncios atuais e side rails automáticos desligados. Este quadro não consulta nem altera esses controles da conta.</p>';
	echo '<p>Preferências antigas de arquitetura não suspendem as unidades manuais. A migração administrativa preserva uma cópia do valor anterior e solicita a limpeza do cache uma vez; confirme separadamente a propagação do CDN e o HTML anônimo publicado. Relatórios históricos e o registro dos controles da conta continuam preservados.</p>';
	if ( function_exists( 'go_verge_adsense_topscroll_baseline_applied' ) && function_exists( 'go_verge_adsense_topscroll_config' ) ) {
		...
		echo '<p><strong>Reset local de Top Scroll:</strong> ' . ( go_verge_adsense_topscroll_baseline_applied() ? 'registrado uma vez.' : 'preset aplicado em memória; persistência administrativa pendente.' ) . ' Preferência efetiva da unidade: <code>' . ( ! empty( $topscroll['enabled'] ) ? 'habilitada' : 'desabilitada' ) . '</code>; teto local <code>' . esc_html( (string) ( $topscroll['frequency_max'] ?? 0 ) ) . '</code> por 24h. A chave global e o consentimento continuam valendo.</p>';
	...
	$marker = get_option( 'go_verge_ads_manual_architecture_v1', false );
