...
function go_verge_render_desk_subjects() {
	...
	foreach ( go_verge_publisher_navigation() as $group ) {
		...
		foreach ( array( 'platform' => 'Plataformas', 'topic' => 'Seções' ) as $kind => $label ) {
			...
			echo '<div class="od-desk-subject-row"><span class="od-desk-subject-label">' . esc_html( $label ) . '</span><nav class="od-desk-subjects" aria-label="' . esc_attr( $label . ' de ' . $group['label'] ) . '">';
			$shown = false;
