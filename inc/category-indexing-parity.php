...
const GO_VERGE_CATEGORY_INDEXING_PARITY_VERSION = '2026-09-18-v5-dynamic-desks';
...
function go_verge_category_parity_slugs() {
	...
	return array(
		...
		'tecnologia', 'apps-software', 'celulares', 'ciencia', 'hardware', 'ia', 'notebooks', 'tvs-e-monitores',
	);
}
 ...
 * noise out of Search, but it also meant a genuinely new desk (for example a
 * new Technology child) could render publicly while remaining excluded from
 * robots/sitemaps until the next theme release. Known blueprint desks activate
 * from their first published story; new descendants of a canonical pillar
 * activate from their first published story as long as they live beneath a
 * canonical editorial pillar; free-form top-level taxonomy noise stays excluded.
 */
function go_verge_category_parity_is_editorial_term( $term ) {
	...
	if ( in_array( $slug, go_verge_category_parity_slugs(), true ) ) {
		return (int) $term->count >= 1 || in_array( $slug, go_verge_category_parity_root_slugs(), true );
	}
	if ( (int) $term->count < 1 ) {
		return false;
	}
...
function go_verge_category_parity_should_force_index() {
	...
	if ( is_category() ) {
		 ...
		 * taxonomy noise. Durable root hubs stay indexable as before, while child
		 * desks become indexable from their first published story so robots and the
		 * XML sitemap never disagree during a new desk launch. */
...
function go_verge_category_parity_deploy() {
	...
	foreach ( go_verge_category_parity_slugs() as $slug ) {
		...
		/* Every canonical editorial desk becomes public with its first published
		 * story. Empty terms stay dormant, while stale noindex/canonical metadata is
		 * cleared immediately for newly launched child desks. */
		if ( ! in_array( $slug, $root_slugs, true ) && (int) $term->count < 1 ) {
			continue;
		}
