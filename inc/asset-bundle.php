 ...
 *   - The bundle occupies the position of the LAST folded sheet. Anything a
 *     plugin queued between two theme sheets therefore ends up before the whole
 *     bundle, which is the same relative outcome the late theme layers already
 *     had, and never lets plugin CSS jump ahead of the design system.
...
if ( ! defined( 'GO_VERGE_STYLE_BUNDLE_FORMAT_VERSION' ) ) {
	define( 'GO_VERGE_STYLE_BUNDLE_FORMAT_VERSION', '2' );
}
...
function go_verge_build_style_bundle() {
	...
	if ( ! is_file( $file ) ) {
		...
		go_verge_style_bundle_prune( $paths['dir'], basename( $file ) );
		} finally {
			go_verge_release_job_lock( $bundle_lock );
		}
	}

	$bundle_url = $paths['url'] . '/' . basename( $file );
	wp_register_style( GO_VERGE_STYLE_BUNDLE_HANDLE, $bundle_url, array(), null );
	$GLOBALS['go_verge_style_bundle_url'] = $bundle_url;

	/* Only after a usable bundle exists do we neutralise the source handles. */
	foreach ( $entries as $entry ) {
		$item = $entry['item'];
		if ( false !== $entry['path'] ) {
			$item->src = false;
		...
		if ( ! empty( $entry['inline'] ) ) {
			$item->extra['after'] = array();
		}
	}

	$last  = $folded[ count( $folded ) - 1 ];
	$queue = array();
	$done  = false;
	foreach ( $styles->queue as $handle ) {
		if ( $handle === $last ) {
			$queue[] = GO_VERGE_STYLE_BUNDLE_HANDLE;
			$done    = true;
			continue;
		}
		if ( in_array( $handle, $folded, true ) ) {
			continue;
		}
		$queue[] = $handle;
	}
	if ( ! $done ) {
		array_unshift( $queue, GO_VERGE_STYLE_BUNDLE_HANDLE );
	}
	$styles->queue = $queue;
	$styles->to_do = array();
}
 ...
 * fingerprints. Retaining 32 variants prevents an 8-file eviction loop from
 * rebuilding valid bundles while still bounding disk growth.
 *
