...
function go_verge_smart_sitemap_term_eligible( $term ) {
	if ( ! ( $term instanceof WP_Term ) || (int)$term->count < 1 ) { return false; }
	$provider = go_verge_smart_sitemap_seo_provider();
	 ...
	 * contract, not by stale per-term plugin metadata. Frontend robots already
	 * forces these desks to index/follow from their first published story; the
	 * sitemap must make the identical decision or a freshly launched desk can
	 * render indexable while disappearing from XML. */
	...
	if ( 'category' === $term->taxonomy ) {
		/* Editorial desks are a controlled vocabulary. A newly launched canonical
		 * desk must be discoverable as soon as it has its first published story; the
		 * old two-story threshold was appropriate for free-form tags, but caused new
		 * categories such as Doramas, Música, Apps e Software and Celulares to vanish
		 * from /sitemap-categories.xml during launch. Empty, technical, legacy and
		 * redirected category objects remain excluded below. */
		if ( (int) $term->count < 1 || 'interno' === $term->slug ) { return false; }
