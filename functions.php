 ...
 */
define( 'GO_VERGE_BRAND_NAME', 'Game Overdrive' );
 ...
 * domain. */
define( 'GO_VERGE_BRAND_ALTERNATE_NAME', 'Overdrive' );
...
/**
 * Preload the two faces that actually paint the page.
 *
 * The previous version preloaded Space Grotesk — a family the 4.7.1 rebrand
 * stopped using for anything above the fold — and then skipped itself entirely
 * on mobile and on any page with a hero image. Since 95% of readers are on a
 * phone and nearly every template has a hero, it preloaded nothing, for anyone,
 * while the two families that carry every headline and every paragraph were
 * being fetched from a third-party origin behind an async stylesheet.
 *
 * Two files, both from this origin, both on the connection the document is
 * already using: Barlow Condensed 700 (33 KB) for the headline and the Source
 * Sans 3 variable (78 KB) for the body — one file covering 200-900 where the
 * old Google request fetched four static weights.
 *
...
function go_verge_preload_primary_font() {
	...
	foreach ( array( 'barlow-condensed/barlow-condensed-700-latin.woff2', 'source-sans-3/source-sans-3-variable-latin.woff2' ) as $file ) {
		printf( '<link rel="preload" href="%s" as="font" type="font/woff2" crossorigin>' . "\n", esc_url( $fonts . $file ) );
	}
	if ( is_front_page() || is_home() ) {
		printf( '<link rel="preload" href="%s" as="font" type="font/woff2" crossorigin media="(min-width:901px)">' . "\n", esc_url( $fonts . 'barlow-condensed/barlow-condensed-600-latin.woff2' ) );
	}
}
...
function go_verge_preload_lcp_image() {
	...
	if ( ! empty( $GLOBALS['go_verge_lcp_preload_image_id'] ) ) {
		...
		$sizes    = '(max-width: 767px) calc(100vw - 32px), (max-width: 1279px) 66vw, 860px';
...
function go_verge_backfill_content_image_dimensions( $content ) {
	...
	return preg_replace_callback(
		'/<img\b[^>]*>/i',
		static function ( $match ) use ( $home_host, &$lookups ) {
			$tag = $match[0];

			/* Most content images are below the article hero and should stay lazy.
			 * WordPress can, however, promote an early image with fetchpriority=high
			 * (notably when a post has no featured image). Never combine that signal
			 * with loading=lazy: the browser would delay the very resource WordPress
			 * identified as important. High priority wins and is protected from a
			 * second lazy-loading layer such as LiteSpeed. */
			$is_high  = (bool) preg_match( '/\bfetchpriority\s*=\s*["\']high["\']/i', $tag );
			$is_eager = (bool) preg_match( '/\bloading\s*=\s*["\']eager["\']/i', $tag );

			if ( $is_high && preg_match( '/\bloading\s*=\s*["\']lazy["\']/i', $tag ) ) {
				$tag = preg_replace( '/\bloading\s*=\s*(["\'])lazy\1/i', 'loading="eager"', $tag, 1 );
				$is_eager = true;
			}
			if ( ! preg_match( '/\bdecoding\s*=/i', $tag ) ) {
				$tag = preg_replace( '/\s*\/?\>$/', ' decoding="async">', $tag );
			}
			if ( ! preg_match( '/\bloading\s*=/i', $tag ) ) {
				$tag = preg_replace( '/\s*\/?\>$/', ( $is_high ? ' loading="eager">' : ' loading="lazy">' ), $tag );
				$is_eager = $is_high;
			}
			/* Lazy editorial images are definitively non-LCP. Lower their network
			 * priority so the hero, CSS and early ad bootstrap win scarce mobile
			 * bandwidth. Never override an explicit priority from WordPress/template. */
			if ( ! $is_high && ! $is_eager && ! preg_match( '/\bfetchpriority\s*=/i', $tag ) ) {
				$tag = preg_replace( '/\s*\/?\>$/', ' fetchpriority="low">', $tag );
			}
			if ( ( $is_high || $is_eager ) && ! preg_match( '/\bdata-no-lazy\s*=/i', $tag ) ) {
				$tag = preg_replace( '/\s*\/?\>$/', ' data-no-lazy="1">', $tag );
			}

			$has_width  = (bool) preg_match( '/\bwidth\s*=\s*["\'][^"\']+["\']/i', $tag );
			$has_height = (bool) preg_match( '/\bheight\s*=\s*["\'][^"\']+["\']/i', $tag );
			...
			/* Classic/custom HTML sometimes loses wp-image-{ID}. Recover local
			 * Media Library images by URL, but cap lookups to keep long stories cheap. */
			if ( ! $attachment_id && $src && $lookups < 12 ) {
				$src_host = strtolower( (string) wp_parse_url( $src, PHP_URL_HOST ) );
				if ( '' === $src_host || '' === $home_host || $src_host === $home_host ) {
					++$lookups;
					$attachment_id = absint( attachment_url_to_postid( $src ) );
				}
			}
			if ( ! $attachment_id ) {
				return $tag;
			...
			/* Match the actual intermediate file when the editor inserted a crop;
			 * using the original aspect ratio for a cropped URL can itself cause CLS. */
			if ( $src ) {
				$src_file = basename( (string) wp_parse_url( $src, PHP_URL_PATH ) );
				foreach ( (array) ( $meta['sizes'] ?? array() ) as $size ) {
					if ( ! empty( $size['file'] ) && basename( (string) $size['file'] ) === $src_file ) {
...
/**
 * Keep AVIF out of the media library while this host cannot serve it.
 *
 * Two independent failures, both verified live on 18/08/2026:
 *
 * 1. Every `.avif` is answered with `Content-Type: text/plain` next to
 *    `X-Content-Type-Options: nosniff`. Browsers still paint the image because
 *    they sniff the bytes; social crawlers and Google's image pipeline validate
 *    the header, so the article is shared with no preview and enters Discover
 *    with no large-image asset. Discover is two thirds of this site's traffic.
 * 2. The image editor cannot resize AVIF, so an AVIF upload produces no
 *    sub-sizes at all: one full-size file is served to every screen width, with
 *    no srcset.
 *
 * Neither is fixable from PHP — the first is a server MIME map, the second is a
 * missing encoder. Blocking the upload is what stops new articles from being
 * published into that state. JPEG and PNG have no such problem and their
 * sub-sizes are converted to WebP above, which is the lighter result anyway.
 *
 * Once the server serves image/avif correctly AND generates AVIF sub-sizes,
 * re-enable with:
 *
...
function go_verge_explain_blocked_avif( $file ) {
	if ( apply_filters( 'go_verge_allow_avif_uploads', false ) || empty( $file['name'] ) ) {
		return $file;
	}
	...
	$file['error'] = __( 'Este servidor entrega arquivos AVIF com o tipo errado, o que quebra a prévia em redes sociais e a imagem grande no Google Discover. Suba a imagem em JPEG ou PNG: o site converte os recortes para WebP automaticamente.', 'go-verge' );
	...
	if ( is_singular( 'post' ) ) {
		...
		$specific_signals = array(
			...
			'technology' => array(
				...
				'inteligencia-artificial', 'ia', 'internet', 'seguranca-digital', 'aplicativos', 'noticias-de-tecnologia',
			),
		);
