...
func _run() -> void:
	...
	var kit_scr := _screen()
	var kit_sc := kit_scr.scroll()
	kit_sc.scroll_vertical = 700
	await _frames(4)
	await _shot("06a1_uniforme_modelos")
	kit_scr.set("_part", "shirt")
	kit_scr.set("_group", 4)
	kit_scr.refresh()
	await _frames(4)
	kit_sc.scroll_vertical = 700
	await _frames(4)
	await _shot("06a2_uniforme_estampas")
	kit_scr.set("_part", "details")
	kit_scr.refresh()
	await _frames(4)
	kit_sc.scroll_vertical = 700
	await _frames(4)
	await _shot("06a3_uniforme_detalhes")
	kit_sc.scroll_vertical = 0
	...
	_screen().set("_which", "third")
	_screen().set("_part", "models")
	_screen().refresh()
	await _frames(4)
	await _shot("06d3_uniforme_terceiro")
	_screen().set("_which", "gk")
	_screen().refresh()
	await _frames(4)
	await _shot("06d5_uniforme_goleiro")
	UIManager.back()
	...
	_screen().set("_tab", "history")
	_screen().refresh()
	await _frames(6)
	await _shot("14h_campeoes")
	...
	_screen().set("_tab", "search")
	_screen().set("_origin", "nat")
	_screen().refresh()
	await _frames(6)
	await _shot("18b_mercado_nacionais")
	_screen().set("_tab", "scout")
	_screen().set("_origin", "nat")
	_screen().refresh()
	Scouting.send_mission(w, "nat", -1, 99)
	_screen().refresh()
	await _frames(6)
	await _shot("18c_olheiros")
	_screen().set("_origin", "all")
	...
	await _frames(8)
	await _shot("32_base")
	_screen().set("_tab", "u20")
	_screen().refresh()
	await _frames(6)
	await _shot("33_sub20")
	YouthManager.run_trial(w)
	for yt in ["u17", "games", "scout", "grads"]:
		_screen().set("_tab", yt)
		_screen().refresh()
		await _frames(6)
		await _shot("33_base_" + yt)
