 ...
 *
 * The chain from <main> down to the prose root carries the same tag and class
 * list on every story. Google Auto Ads learns its article structure as a CSS
 * path through these ancestors (tag + every class of each node), so a per-post
 * class here - post_class() categories, tags, services, format, thumbnail -
 * turned the learned path into "articles of that category only". Story-type
 * modifiers (review, critique, guide, premium...) live on <body> instead; see
 * go_verge_single_clean_body_class().
 */
...
/* Phones: Top Display as an exact 320x100 under the byline, before the hero
 * (see go_verge_ads_masthead_phone_mode()). Tablets/desktop keep header.php. */
$od_phone_masthead = function_exists( 'go_verge_ads_render_masthead_phone' ) && 'inline' === go_verge_ads_masthead_phone_mode();
?>
<main id="primary" class="go-main go-single-page">
	<article class="go-single od-article">
		<?php if ( $c['is_scored'] && $c['has_hero'] ) : ?>
					...
					<div class="go-scored-masthead__copy">
						...
						<div class="go-scored-masthead__footer">
							...
							<div class="go-scored-masthead__actions" aria-label="<?php esc_attr_e( 'Ações da matéria', 'go-verge' ); ?>">
								...
								<button type="button" class="go-scored-masthead__action go-scored-masthead__action--text" data-go-scored-copy data-share-url="<?php echo esc_url( $c['permalink'] ); ?>"><?php esc_html_e( 'Copiar link', 'go-verge' ); ?></button>
			...
			<?php if ( $od_phone_masthead ) { go_verge_ads_render_masthead_phone( 'od-article__top-display' ); } ?>
		...
		<?php else : ?>
			<div class="od-article__opening">
				...
				<?php if ( $od_phone_masthead && $c['has_hero'] ) { go_verge_ads_render_masthead_phone( 'od-article__top-display' ); } ?>
