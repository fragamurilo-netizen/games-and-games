...
function go_verge_seo_site_name() {
	return defined( 'GO_VERGE_BRAND_NAME' ) ? GO_VERGE_BRAND_NAME : 'Overdrive';
}

/**
 * Public fallback names for Google site-name selection.
 *
 * "Overdrive" is the preferred publisher/site name (`name`). Google also
 * supports `alternateName`, so the former "Game Overdrive" identity remains
 * available as a transition/legacy signal without competing with the canonical
 * brand.
 *
 * Order matters: preferred `name` is Overdrive; `alternateName` is
 * Game Overdrive. Google may still choose another representation automatically.
 *
 * @return string[]
 */
function go_verge_seo_site_alternate_names() {
	...
	$names   = (array) apply_filters(
		...
		array( defined( 'GO_VERGE_BRAND_ALTERNATE_NAME' ) ? GO_VERGE_BRAND_ALTERNATE_NAME : 'Game Overdrive' )
	);
...
/**
 * Metadata contract for virtual documents supplied by plugins.
 * The theme remains the single renderer of native OG/Twitter/canonical tags.
 */
function go_verge_seo_document_metadata() {
	return apply_filters( 'go_verge_seo_document_metadata', array(
...
function go_verge_seo_head() {
	...
	$metadata    = go_verge_seo_document_metadata();
...
function go_verge_news_readiness_health_test() {
	...
	if ( ! $publication_ok ) {
		$issues[] = esc_html__( 'O nome da publicação no sitemap de notícias precisa ser “Overdrive”.', 'go-verge' );
	}
	if ( ! $room_ok ) {
