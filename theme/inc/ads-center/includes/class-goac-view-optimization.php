 ...
 * contrato publicado pelo tema: inventário manual e Auto ads da conta.
...
final class GOAC_View_Optimization {
	...
	/** Both contracts retain the same fixed manual inventory and evidence guards. */
	private static function fixed_inventory_mode() {
		return in_array( self::delivery_mode(), array( 'manual_overlays', 'hybrid' ), true );
	}

	/** A regra de conta que acompanha cada recomendação do motor manual. */
	private static function account_rule() {
		return 'Os formatos automáticos são definidos na conta. Avalie carga e exclusões na prévia e nos experimentos do AdSense, junto do inventário manual. O painel não altera nem confirma esses controles.';
	}
	...
	private static function controls_card( $day, array $cfg ) {
		...
		GOAC_UI::card_open( 'Entrega do tema', '<span class="goac-pill ok">MANUAIS + AUTO ADS</span>' );
		echo '<p class="goac-muted goac-section-intro"><strong>Inventário manual e Auto ads.</strong> O tema respeita os templates, dispositivos e bloqueios configurados. Os formatos automáticos são definidos na conta AdSense. Os indicadores de RPM são diagnósticos e não ajustam automaticamente o volume manual.</p>';
	...
	/** Manual record of account controls; never a live API assertion. */
	private static function controls_mirror( $day, array $cfg ) {
		...
		echo '<p class="goac-muted goac-section-intro">Registre os controles conferidos na conta AdSense. A API não expõe esta configuração; o registro não ativa formatos nem comprova entrega.</p>';
		...
		echo '<label><span>Auto ads geral</span>' . self::tri_select( 'auto_ads', $cfg['auto_ads'] ) . '</label>';
		...
		echo '<label><span>Side rails automáticos</span>' . self::tri_select( 'side_rails_ads', $cfg['side_rails_ads'] ) . '</label>';
	...
	private static function recommendations( array $cur, array $b, array $long, array $sample, array $breakdowns, $partial, array $cfg, array $quality = array() ) {
		...
		if ( self::fixed_inventory_mode() && ( $partial || ! $sample['enough'] || ( isset( $quality['ready'] ) && ! $quality['ready'] ) ) ) {
	...
	private static function actions_card( array $actions, array $sample, array $current, array $bench, array $cfg ) {
		...
		echo '</div>';
		if ( ! self::fixed_inventory_mode() && empty( $cfg['configured'] ) ) { echo '<p class="goac-opt-config-warning"><strong>Para o painel escrever “6 → 7” em vez de “uma marca”:</strong> preencha “Minha configuração atual do AdSense” acima.</p>'; }
		...
		echo '</div>';
		echo '<p class="goac-footnote">Regra do motor: <strong>uma mudança de monetização por vez</strong>. Compare posições identificáveis e altere uma variável de densidade, elegibilidade ou carregamento. Use a prévia e experimentos na conta para avaliar Auto ads junto do inventário manual. O registro da conta é um espelho operacional, não uma confirmação pela API.</p>';
		GOAC_UI::card_close();
	}
