...
final class GOAC_View_Delivery {
	...
	private static function architecture_card( $contract ) {
		...
		$mode_note = '<strong>Manuais + Auto ads:</strong> o loader único atende o inventário do tema e os formatos automáticos definidos na conta. Confira formatos, carga e exclusões na prévia do AdSense. Esta tela não confirma os controles atuais do Google.';
	...
	private static function revenue_split_card( $contract ) {
		...
		echo '</tbody></table></div>';
		if ( $has_contract ) {
			echo '<p class="goac-footnote"><strong>Como ler:</strong> o residual não comprova quais formatos foram veiculados. Confira receita, impressões e controles por formato antes de atribuir a mudança. Compare somente períodos com escopo, moeda e atualização compatíveis.</p>';
		}
		GOAC_UI::card_close();
	}
