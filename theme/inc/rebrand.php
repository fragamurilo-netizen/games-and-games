...
function go_verge_rebrand_copy( $value ) {
	...
	/*
	 * Migrate only the human-facing display name. Technical identifiers such as
	 * gameoverdrive.com.br, contato@gameoverdrive.com.br, /game-overdrive/ and
	 * social handles contain no whitespace and are therefore left untouched.
	 */
	return preg_replace( '/\bgame(?:\s|&nbsp;|&#160;)+overdrive\b/iu', GO_VERGE_BRAND_NAME, $value );
}
...
add_filter( 'option_blogdescription', 'go_verge_rebrand_copy', 999 );
...
function go_verge_rebrand_document_title_parts( $parts ) {
	...
	$post_id    = function_exists( 'get_queried_object_id' ) ? absint( get_queried_object_id() ) : 0;
	$historical = $post_id && go_verge_rebrand_is_historical_post( $post_id );
	foreach ( $parts as $key => $value ) {
		if ( 'site' !== $key && $historical ) {
			continue;
		}
 ...
 *
 * "Overdrive" is primary. "Game Overdrive" is retained only as a legacy
 * human-readable fallback while search engines and external references converge.
...
/** Keep visible post/page titles aligned without mutating historical rename coverage. */
function go_verge_rebrand_visible_title( $title, $post_id = 0 ) {
	if ( is_admin() ) {
		return $title;
	}
...
add_filter( 'the_title', 'go_verge_rebrand_visible_title', 9998, 2 );

/** Keep archive cards and manually written excerpts on the current public brand. */
function go_verge_rebrand_visible_excerpt( $excerpt, $post = null ) {
	if ( is_admin() ) {
		return $excerpt;
	}
	...
	return go_verge_rebrand_copy( $excerpt );
}
add_filter( 'get_the_excerpt', 'go_verge_rebrand_visible_excerpt', 9998, 2 );

/** Navigation labels are brand-facing copy; destinations themselves stay untouched. */
function go_verge_rebrand_menu_title( $title ) {
	return go_verge_rebrand_copy( $title );
}
add_filter( 'nav_menu_item_title', 'go_verge_rebrand_menu_title', 9998 );
...
function go_verge_apply_complete_rebrand_v3() {
	...
	/* Rank Math gets Overdrive as primary plus Game Overdrive as the intentional legacy alternateName. */
 ...
 *
 * The canonical publisher/site name is Overdrive. Game Overdrive remains
 * available as alternateName for Google and other consumers during transition.
 */
...
/**
 * 5.5.11 — make Overdrive the canonical publisher/site name again.
 * Game Overdrive remains available as alternateName during the brand transition.
 * Runs on admin_init so replacing the active theme ZIP is enough; no theme
 * deactivation/reactivation is required.
 */
function go_verge_apply_overdrive_identity_v3() {
	if ( (string) get_option( 'go_overdrive_identity_v3', '' ) === (string) GO_VERGE_VERSION ) {
		return;
	}
	...
	update_option( 'go_verge_news_publication_name', GO_VERGE_BRAND_NAME, false );
	$description = (string) get_option( 'blogdescription', '' );
	$clean_description = go_verge_rebrand_copy( $description );
	if ( $clean_description !== $description ) {
		update_option( 'blogdescription', $clean_description );
	}
	...
	update_option( 'go_overdrive_identity_v3', GO_VERGE_VERSION, false );
	if ( function_exists( 'go_verge_purge_all_page_cache' ) ) {
		go_verge_purge_all_page_cache();
	}
}
add_action( 'admin_init', 'go_verge_apply_overdrive_identity_v3', 120 );
add_action( 'after_switch_theme', 'go_verge_apply_overdrive_identity_v3', 120 );
...
function go_verge_rebrand_register_health_test( $tests ) {
	$tests['direct']['go_verge_brand_consistency'] = array(
		'label' => __( 'Consistência da marca Overdrive', 'go-verge' ),
...
function go_verge_rebrand_health_test() {
	...
	if ( defined( 'RANK_MATH_VERSION' ) ) {
		...
		$rank_math_ok     = is_array( $rank_math_titles )
			...
			&& GO_VERGE_BRAND_ALTERNATE_NAME === trim( (string) ( $rank_math_titles['website_alternate_name'] ?? '' ) );
	}
	$good = $site_name_ok && $news_name_ok && $rank_math_ok && 0 === $unexpected;

	return array(
		'label'       => $good
			? __( 'A marca pública está consistente como Overdrive', 'go-verge' )
