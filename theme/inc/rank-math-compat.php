 ...
 * Open Graph repairs
 *
 * These exist because the theme overrides Rank Math's social image (above) to
 * a larger Discover-ready asset. Measured on production 02/09/2026:
 *
 *   /games/ (override does NOT run)   og:image + width + height + type
 *   /dawnwalker-.../ (override RUNS)  og:image + secure_url + alt, NO width,
 *                                     NO height, NO type
 *
 * Rank Math derives those three tags from the image record it resolved itself,
 * so replacing only the URL through `rank_math/opengraph/facebook/image` leaves
 * them unset and the tags are never printed. The existing
 * `.../og_image_width` and `.../og_image_height` filters above cannot help:
 * a filter on a tag that is never emitted never runs.
 *
 * WhatsApp and Facebook use these dimensions to lay out a large preview on the
 * first share instead of re-crawling, and WhatsApp is a primary distribution
 * surface for this newsroom, so the tags are worth restoring.
 * ---------------------------------------------------------------------- */

/**
 * Whether this response can carry the Open Graph image repair.
 *
 * Scoped to exactly the condition that loses the tags: Rank Math owning the
 * head on a singular post, which is the only place the image override runs.
 *
 * @return bool
 */
function go_verge_rank_math_og_repair_applies() {
...
/**
 * Inject the missing og:image dimensions into a buffered wp_head fragment.
 *
 * Deterministic and idempotent: it only adds tags, never removes or rewrites
 * one, and it refuses when the document already carries `og:image:width`. If a
 * future Rank Math release starts emitting them, this becomes a no-op instead
 * of a duplicate.
 *
 * @param string $head Buffered wp_head output.
 * @return string
 */
function go_verge_rank_math_repair_og_image_block( $head ) {
	if ( '' === $head || false === stripos( $head, 'og:image' ) ) {
		return $head;
	}
	/* Rank Math already emitted them, or another integration did. Do nothing. */
	if ( preg_match( '#property=(["\'])og:image:width\1#i', $head ) ) {
		return $head;
	}
	...
	/*
	 * Only annotate when the printed og:image really is the asset we chose.
	 * Comparing the file name keeps this correct through esc_url()/entity
	 * encoding differences without matching a different image by accident.
	 */
	$basename = wp_basename( parse_url( $url, PHP_URL_PATH ) ?: $url );
	if ( '' === $basename || false === strpos( $head, $basename ) ) {
		return $head;
	}
	...
	/* Place them immediately after the og:image (or its secure_url twin) so the
	 * block stays in the conventional order consumers expect. */
	$anchor = '#(<meta[^>]+property=(["\'])og:image:secure_url\2[^>]*>\s*)#i';
	if ( ! preg_match( $anchor, $head ) ) {
		$anchor = '#(<meta[^>]+property=(["\'])og:image\2[^>]*>\s*)#i';
	}
	$repaired = preg_replace( $anchor, '$1' . str_replace( '$', '\$', $tags ), $head, 1 );

	return is_string( $repaired ) ? $repaired : $head;
}

/**
 * Open the scoped head buffer.
 *
 * The only other wp_head buffer in the theme is the AdSense loader normalizer
 * (-PHP_INT_MAX .. PHP_INT_MAX), so this one nests entirely inside it and is
 * closed long before that one reads its own buffer.
 *
 * @return void
 */
function go_verge_rank_math_begin_og_repair() {
	if ( ! go_verge_rank_math_og_repair_applies() ) {
		return;
	}
	$GLOBALS['go_verge_og_repair_base'] = ob_get_level();
	ob_start();
}
add_action( 'wp_head', 'go_verge_rank_math_begin_og_repair', 0 );

/**
 * Repair and flush the scoped head buffer.
 *
 * Uses the same defensive level accounting as inc/ads/loader.php: a nested
 * buffer opened by another callback is flushed into ours first, and a level
 * mismatch aborts rather than popping a buffer this function does not own.
 *
 * @return void
 */
function go_verge_rank_math_end_og_repair() {
	if ( ! isset( $GLOBALS['go_verge_og_repair_base'] ) ) {
		return;
	}
	$base = (int) $GLOBALS['go_verge_og_repair_base'];
	unset( $GLOBALS['go_verge_og_repair_base'] );

	while ( ob_get_level() > $base + 1 ) {
		ob_end_flush();
	}
	if ( ob_get_level() !== $base + 1 ) {
		return;
	}

	$head = (string) ob_get_clean();
	echo go_verge_rank_math_repair_og_image_block( $head ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- buffered wp_head output; injected values are absint().
}
add_action( 'wp_head', 'go_verge_rank_math_end_og_repair', 9 );
