...
function go_verge_render_editorial_hub_page( $title, $aliases, $fragments = array(), $latest_title = '', $editorial_zone = '', $access_sections = array() ) {
	...
	$access_requested = isset( $_GET['subcategoria'] ) && is_scalar( $_GET['subcategoria'] )
		? sanitize_title( wp_unslash( (string) $_GET['subcategoria'] ) )
	...
	$requested   = isset( $_GET['editoria'] ) && is_scalar( $_GET['editoria'] ) ? sanitize_title( wp_unslash( (string) $_GET['editoria'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	$selected    = null;
...
function go_verge_render_archive_sidebar( $modules ) {
	...
	<aside class="go-archive-aside" aria-label="<?php esc_attr_e( 'Destaques da editoria', 'go-verge' ); ?>">
		<?php /* Clever 300x250 at the top of the sidebar (home and archives only). */ ?>
		<?php if ( function_exists( 'go_verge_clever_render_sidebar_slot' ) ) { go_verge_clever_render_sidebar_slot(); } ?>
	...
	if ( $contextual_next_rendered && ! function_exists( 'go_verge_single_recommendation_ids' ) ) {
		...
		$verticals = array(
			...
			'technology' => array(
				...
				'aliases'   => array( 'tecnologia', 'celulares', 'mobilidade', 'internet', 'hardware', 'notebooks', 'tvs-e-monitores', 'eletrodomesticos', 'casa-inteligente', 'wearables', 'apps-software', 'ia', 'ciencia', 'servico' ),
