 ...
 */
const GO_VERGE_SMART_SITEMAP_OUTPUT_VERSION = '5.6.3-provider-index';
...
function go_verge_smart_sitemap_index_children() {
	...
	return apply_filters( 'go_verge_smart_sitemap_index_children', $children );
}
...
function go_verge_smart_sitemap_send_xml( $xml, $route ) {
	...
	$etag='"go-sm-'.md5($xml).'"';
	/* Fresh membership and plugin-provided index children can change without an
	 * editorial save. Their body ETag is authoritative; the post clock is not. */
	$last = in_array( $route['kind'], array( 'fresh', 'index' ), true ) ? 0 : go_verge_smart_sitemap_route_lastmod( $route );
...
function go_verge_smart_sitemap_render_route( $route ) {
	...
	$epoch=go_verge_smart_sitemap_epoch();$key='go_sm_xml_'.md5(GO_VERGE_SMART_SITEMAP_OUTPUT_VERSION.'|'.$epoch.'|'.wp_json_encode($route));$xml=get_transient($key);
