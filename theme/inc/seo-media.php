...
function go_verge_auto_image_alt( $attr, $attachment ) {
	 ...
	 */
	if ( ( ! empty( $attr['role'] ) && in_array( strtolower( (string) $attr['role'] ), array( 'presentation', 'none' ), true ) )
...
function go_verge_auto_content_image_alt( $content ) {
	if ( ! go_verge_auto_image_alt_enabled() || ! is_string( $content ) || false === stripos( $content, '<img' ) || ! class_exists( 'WP_HTML_Tag_Processor' ) ) {
		return $content;
	}
	...
	while ( $processor->next_tag( 'IMG' ) ) {
		/* Parse real attributes, including unquoted values, without touching
		 * data-alt, script text or a > character inside a quoted attribute. */
		if ( in_array( strtolower( (string) $processor->get_attribute( 'role' ) ), array( 'presentation', 'none' ), true )
			|| 'true' === strtolower( (string) $processor->get_attribute( 'aria-hidden' ) )
			|| null !== $processor->get_attribute( 'data-go-decorative' ) ) {
			continue;
		}
		...
		$description = '';
		if ( preg_match( '/(?:^|\s)wp-image-(\d+)(?:\s|$)/i', (string) $processor->get_attribute( 'class' ), $id ) ) {
		...
		if ( '' !== $description ) {
			$processor->set_attribute( 'alt', $description );
		}
	}
