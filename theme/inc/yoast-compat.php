...
/** Apply the same final privacy boundary as the native and Rank Math heads. */
function go_verge_yoast_final_restrictions( $robots ) {
	if ( ! is_array( $robots ) ) { return $robots; }
	if ( function_exists( 'go_verge_robots_restricted_view' ) && go_verge_robots_restricted_view() ) {
		$robots['index'] = 'noindex';
	}
	if ( 'noindex' === (string) ( $robots['index'] ?? '' ) ) {
		unset( $robots['max-snippet'], $robots['max-image-preview'], $robots['max-video-preview'] );
	}
	return $robots;
}
add_filter( 'wpseo_robots_array', 'go_verge_yoast_final_restrictions', PHP_INT_MAX );
