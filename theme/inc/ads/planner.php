 ...
 * opportunity after two substantial paragraphs; the configured A-ladder distributes the
 ...
 * the advance body target. A reached or engaged nearby reserve can qualify
 * within structural capacity. Their purpose is to recover real-world loss: an earlier
...
function go_verge_ads_planner_reserve_candidates( $candidates, $prime, $selected, $gap_words, $gap_height, $limit = 2 ) {
	...
	$available_ids = max( 0, go_verge_ads_planner_contract_capacity() - 1 - count( (array) $selected ) );
...
function go_verge_ads_plan_article( $html, $article_type = '' ) {
	...
	$a_capacity = min( $a_slots, max( 0, $total_capacity - ( $prime ? 1 : 0 ) ) );
	 ...
	 * only admits a reserve once it is visible or predictively imminent; every density rule still applies.
	 */
