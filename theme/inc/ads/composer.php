...
/**
 * Insert Prime P1 + the configured A-ladder into the final article HTML.
 *
...
function go_verge_ads_compose_article_inventory( $content ) {
	...
	if ( preg_match( '/\bdata-go-ad-placement\s*=\s*(?:["\'](?:article-prime|article-a[1-9][0-9]*)["\']|(?:article-prime|article-a[1-9][0-9]*)(?=\s|>))/i', $content ) ) {
		return $content;
	}
	...
	foreach ( $selected as $candidate ) {
		...
		if ( ! preg_match( '/^(?:article-prime|article-a[1-9][0-9]*)$/', $placement ) || $position < 1 || $position > strlen( $content ) ) {
			continue;
		}
