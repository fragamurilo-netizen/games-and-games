 ...
 *
 * Delivery model: publisher inventory + account-managed Auto Ads ("hybrid").
 *
 * The theme owns its manual inventory. Auto Ads placement and formats remain
 * in AdSense; this file does not enable or certify those account settings.
 * Recognized in-page Auto units participate in spacing before a new manual
 * request. Recognized anchors adjust the usable viewport. The runtime never
 * moves, resizes, hides, replaces, requests or refreshes provider-owned units.
 *
...
/*
 * Long-form body extension A7/A8, configured for controlled opt-in.
 *
 * The default production ceiling remains A6 while RPM/Active View recover.
 * Set GO_VERGE_ADS_ARTICLE_MAX_RUNG to 7 or 8 only after unit-level field data
 * proves the deeper rung adds page revenue without degrading viewability/value.
 *
...
/*
 * Production ceiling for the in-article ladder.
 *
 * 5.8.0: the ceiling is the `body_max_rung` engine setting (default 6, editable
 * in wp-admin → Motor de anúncios; see inc/ads/engine-settings.php). Rungs 7-8
 * use the A7/A8 Display units. A GO_VERGE_ADS_ARTICLE_MAX_RUNG constant in
 * wp-config.php still overrides the setting for emergencies.
 */

/*
 * The article rail below the desktop breakpoint, enabled with its configured ID.
 *
	...
	return (array) apply_filters(
		...
		array(
			...
			'mobile' => array(
				...
				/* Premium formats in reading order: Top Scroll, then Top Display,
				 * then Hero Overlay. One still off screen waits for the answer of
				 * the one above it, at most this long; one the reader reaches asks
				 * at once. Timing only; 0 turns it off (see criticalStage()). */
				'critical_stage_ms'       => 1200,
			),
			...
			'desktop' => array(
				...
				/* Desktop keeps its current timing. */
				'critical_stage_ms'       => 0,
			),
...
function go_verge_ads_config() {
	...
	/* 5.8.0 engine settings: body format and surface switches (engine-settings.php). */
	if ( function_exists( 'go_verge_ads_engine_settings' ) ) {
		foreach ( array_keys( go_verge_ads_engine_body_units() ) as $body_placement ) {
			if ( isset( $config['inventory'][ $body_placement ] ) ) {
				$config['inventory'][ $body_placement ] = go_verge_ads_engine_body_unit( $body_placement, $config['inventory'][ $body_placement ] );
			}
		}
		$surface_switches = array(
			'post-content-multiplex' => 'surface_multiplex',
			'article-rail-mobile'    => 'surface_rail_mobile',
			'article-hero-overlay'   => 'hero_overlay',
		);
		foreach ( $surface_switches as $switch_placement => $switch_key ) {
			if ( isset( $config['inventory'][ $switch_placement ] ) && ! go_verge_ads_engine_setting( $switch_key ) ) {
				$config['inventory'][ $switch_placement ]['enabled'] = false;
			}
		}
		unset( $body_placement, $surface_switches, $switch_placement, $switch_key );
	}
	...
	$config['account_formats']['in_page'] = 'account';
	$config['account_formats']['anchor'] = 'account';
	$config['account_formats']['vignette'] = 'account';
	$config['account_formats']['side_rails'] = 'account';
