...
/** Whether the initial defaults have already been persisted. */
function go_verge_adsense_topscroll_baseline_applied() {
	return 1 === (int) get_option( GO_VERGE_ADSENSE_TOPSCROLL_BASELINE_OPTION, 0 );
}

/**
 * Fill missing defaults without replacing an explicit publisher preference.
 * IDs, disabled state, frequency and additional integration fields are retained.
...
function go_verge_adsense_topscroll_baseline_values( $saved ) {
	...
	if ( ! array_key_exists( 'frequency_max', $saved ) ) {
 ...
 * is not read, removed or renamed by this server-side initialization.
 *
 * @param mixed $saved Stored Top Scroll option.
 * @return array<string,mixed>
 */
...
/**
 * Persist missing defaults once, with the original option recoverable.
...
function go_verge_adsense_topscroll_config() {
	...
	$saved   = is_array( $saved ) ? $saved : array();
	$has_enabled = array_key_exists( 'enabled', $saved );
	$has_frequency = array_key_exists( 'frequency_max', $saved );
	...
	$source  = $has_enabled ? 'option' : ( $baseline_pending ? 'baseline' : 'default' );
	$frequency_source = $has_frequency ? 'option' : ( $baseline_pending ? 'baseline' : 'default' );
	if ( ! $has_frequency && 6 !== absint( GO_VERGE_ADSENSE_TOPSCROLL_FREQUENCY_DEFAULT ) ) {
...
/**
 * Top Scroll disclosure: "Publicidade", then a note that the story continues
 * below the unit, so a reader landing on a full-width creative knows the page
 * is not only an ad. The permitted AdSense label stays first and unchanged; the
 * note asks for nothing and points away from the ad. Its ", obrigado" yields on
 * phones narrower than 340px so the line never reaches the close button.
 *
 * @param string $label     Default label markup.
 * @param string $placement Placement key.
 * @return string
 */
function go_verge_adsense_topscroll_label( $label, $placement ) {
	if ( 'topscroll' !== $placement ) {
		return $label;
	}
	return '<span class="go-ad-slot__label">' . esc_html__( 'Publicidade', 'go-verge' )
		. '<span class="go-ad-slot__note"> · ' . esc_html__( 'Seu conteúdo continua abaixo', 'go-verge' )
		. '<span class="go-ad-slot__note-tail">' . esc_html__( ', obrigado', 'go-verge' ) . '</span></span></span>';
}
add_filter( 'go_verge_ads_unit_label', 'go_verge_adsense_topscroll_label', 10, 2 );
