...
(function () {
	...
	function updateLatestActive(clicked) {
		var nav = clicked.closest('[data-go-latest-filters]');
		if (!nav) { return; }
		nav.querySelectorAll('[data-go-latest-filter]').forEach(function (item) {
			var active = item === clicked;
			item.classList.toggle('is-active', active);
	...
	function updateEditorialActive(clicked) {
		var nav = clicked.closest('[data-go-editorial-filters]');
		if (!nav) { return; }
		nav.querySelectorAll('[data-go-editorial-filter], [data-go-review-platform-filter]').forEach(function (item) {
			var active = item === clicked;
			item.classList.toggle('is-active', active);
			if (active) { item.setAttribute('aria-current', 'page'); }
			else { item.removeAttribute('aria-current'); }
	...
	function updateReviewGenreActive(clicked) {
		var scope = clicked.closest('[data-go-review-filter-scope]');
		if (!scope) { return; }
		scope.querySelectorAll('[data-go-review-genre-filter]').forEach(function (item) {
			var active = item === clicked;
			item.classList.toggle('is-active', active);
			if (active) { item.setAttribute('aria-current', 'page'); }
			else { item.removeAttribute('aria-current'); }
	...
	function updateCategoryAccessActive(href) {
		var activeSlug = '';
		try { activeSlug = new URL(href, window.location.href).searchParams.get('subcategoria') || ''; } catch (error) {}
		document.querySelectorAll('[data-go-category-access-filter]').forEach(function (item) {
			var active = (item.getAttribute('data-go-category-access-filter') || '') === activeSlug;
			item.classList.toggle('is-active', active);
			if (active) { item.setAttribute('aria-current', 'page'); }
			else { item.removeAttribute('aria-current'); }
		});
	...
	function replaceFilteredContent(link, kind, pushState) {
		...
		if (!href || !currentRoot || currentRoot.getAttribute('aria-busy') === 'true') { return; }
		...
		fetch(href, {
			...
			.then(function (html) {
				...
				currentRoot.classList.remove('is-filtering');
				if (kind === 'latest' && isElement) { updateLatestActive(link); }
				else if (kind === 'review-latest' && isElement) { updateReviewGenreActive(link); }
				else if (kind === 'category-access') { updateCategoryAccessActive(href); }
				else if (isElement) { updateEditorialActive(link); }
	...
	document.addEventListener('click', function (event) {
		...
		if (latestFilter) {
			event.preventDefault();
			replaceFilteredContent(latestFilter, 'latest');
			return;
		}
		...
		if (reviewGenreFilter && reviewGenreFilter.closest('[data-go-review-filter-scope]')) {
			event.preventDefault();
			replaceFilteredContent(reviewGenreFilter, 'review-latest');
			return;
		}
		...
		if (categoryAccessFilter) {
			event.preventDefault();
			replaceFilteredContent(categoryAccessFilter, 'category-access');
			return;
		}
		...
		if (editorialFilter && editorialFilter.closest('[data-go-editorial-filter-scope]')) {
			event.preventDefault();
			replaceFilteredContent(editorialFilter, 'editorial');
		}
	});
