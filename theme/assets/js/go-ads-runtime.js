/**
 * Overdrive 12.6 — fixed manual baseline with local geometry and latency.
 *
 * DELIVERY MODEL
 * Every in-page opportunity on this site is publisher-owned. Account-level Auto
 * Ads runs with overlay formats only (anchor, vignette); those live outside
 * document flow. The engine only measures recognized anchor geometry to avoid
 * treating covered space as visible; it never moves or counts either format.
 * Nothing here assumes Google will fill a gap the manual ladder leaves behind.
 *
 * WHAT IT DECIDES
...
(function (w, d) {
  ...
  function ensureAnchorWatch() {
    ...
    /* Discover recognized official anchors, including status first assigned
     * after insertion. Only added subtrees and this one marker attribute are
     * inspected. Style/size observation is then limited to the anchor itself;
     * this never scans the whole document on scroll or polls for a provider. */
  ...
  /** Every publisher placement that already holds space in this column. */
  ...
  function inspect() {
    ...
    return {
      ...
      version: VERSION, deliveryModel: 'manual in-page + overlays da conta (âncora/vinheta)',
      trial: trialSignal,
      ...
      manual: records.map(function (r) {
        return { placement: r.placement, slot: r.slot, tier: r.tier, priority: r.critical ? 'critical' : 'normal', surface: r.surface,
          ...
          delivery: { distanceAtRequest: r.distanceAtRequest == null ? null : r.distanceAtRequest, dynamicNearAtRequest: r.dynamicNearAtRequest == null ? null : r.dynamicNearAtRequest,
            ...
            heldForCriticalMs: r.heldMs || 0, heldForFirstPaint: r.paintHeldMs != null, readerEngagedAtRequest: r.engagedAtRequest, governorAtRequest: r.governorAtRequest || null,
  ...
  function explain() {
    ...
    lines.push('Entrega: in-page 100% manual; âncora/vinheta são da conta. A geometria reconhecida da âncora apenas ajusta a viewport útil.');
